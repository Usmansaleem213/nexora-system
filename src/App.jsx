...
export default function App() {
  ...
  const [formData, setFormData] = useState({
    ...
    destination: '', weight: '', service: '', shipment_date: todayStr()
  });
  ...
  const [editFormData, setEditFormData] = useState({
    sender_name: '', sender_address: '', sender_phone: '', sender_email: '',
    receiver_name: '', receiver_address: '', receiver_phone: '', receiver_email: '',
    destination: '', weight: '', service: '', remote_status: 'Non-Remote',
  ...
  const [newUpdate, setNewUpdate] = useState({ location: '', status: 'Picked Up', description: '', custom_time: '' });
  ...
  const handleAddVendor = async (e) => {
    ...
    const { error } = await supabase.from('vendors').insert([{
      name: vendorForm.name, total_due: Number(vendorForm.total_due), total_paid: Number(vendorForm.total_paid),
    ...
    if (!error) {
      ...
      setShowAddVendor(false); fetchVendors();
      setTimeout(() => setVendorMsg(''), 3000);
    } else { setVendorMsg('❌ Error: ' + error.message); }
  };
  const handleEditVendor = (v) => { setEditVendorId(v.id); setEditVendorForm({ name: v.name, total_due: v.total_due || 0, total_paid: v.total_paid || 0, notes: v.notes || '' }); };
  const handleUpdateVendor = async (e) => {
    e.preventDefault();
    const { error } = await supabase.from('vendors').update({ name: editVendorForm.name, total_due: Number(editVendorForm.total_due), total_paid: Number(editVendorForm.total_paid), notes: editVendorForm.notes }).eq('id', editVendorId);
    if (!error) { setEditVendorId(null); fetchVendors(); } else alert('Update failed: ' + error.message);
  };
  const handleDeleteVendor = async (id) => {
    if (window.confirm('Is vendor ko delete karein?')) { await supabase.from('vendors').delete().eq('id', id); fetchVendors(); }
  };
  ...
  const getStatusStyle = (statusValue) => trackingStatuses.find(s => s.value === statusValue) || { color: 'bg-purple-900/60 text-purple-300 border-slate-600', dot: 'bg-slate-500' };
  ...
  const handleSearchTracking = async (e) => {
    ...
    setSelectedAwbShipment(shipment); setTrackingMsg('');
  ...
  const handleAddTrackingUpdate = async (e) => {
    ...
    const { error } = await supabase.from('tracking_updates').insert([{
      awb: selectedAwbShipment.nexora_airwaybill, location: newUpdate.location,
      status: newUpdate.status, description: newUpdate.description, created_at: insertTime
    }]);
    if (!error) {
  ...
  const handleRejectPending = async (id) => {
    if (window.confirm('Is request ko reject karein?')) { await supabase.from('customer_ledgers').delete().eq('id', id); fetchPending(); }
  };
  ...
  const getUniqueCustomers = () => {
    ...
    ledgerData.forEach(item => {
      if (item.sender_name) {
        ...
        if (!customersMap[nameKey]) customersMap[nameKey] = { name: nameKey, phone: item.sender_phone || 'N/A', email: item.sender_email || 'N/A', totalShipments: 0 };
  ...
  // Prefill sender details from a specific customer and jump to New Shipment form
  const handleAddShipmentForCustomer = (customerName) => {
    const match = ledgerData.find(item => item.sender_name && item.sender_name.trim() === customerName);
    const profileMatch = profilesData.find(p => p.full_name && p.full_name.trim() === customerName);
    setFormData({
      sender_name: customerName,
      sender_address: match?.sender_address || '',
      sender_phone: match?.sender_phone || profileMatch?.phone || '',
      sender_email: match?.sender_email || profileMatch?.email || '',
      receiver_name: '', receiver_address: '', receiver_phone: '', receiver_email: '',
      destination: '', weight: '', service: '', shipment_date: todayStr()
    });
    setLabelData(null);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const nexoraTracking = "UT-" + Math.floor(100000000 + Math.random() * 900000000);
    const chosenDate = formData.shipment_date ? new Date(formData.shipment_date + 'T12:00:00').toISOString() : new Date().toISOString();
    const dbPayload = {
  ...
  const handleEditClick = (item) => {
    ...
    setEditFormData({
      ...
      vendor_paid: item.vendor_paid || 0, forwarding_awb: item.forwarding_awb || '', forward_vendor: item.forward_vendor || ''
    });
    setIsModalOpen(true);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    const { error } = await supabase.from('customer_ledgers').update({
      ...
      buying_rate: Number(editFormData.buying_rate), vendor_paid: Number(editFormData.vendor_paid),
      forwarding_awb: editFormData.forwarding_awb, forward_vendor: editFormData.forward_vendor, status: 'approved'
    }).eq('id', editId);
            ...
            {[
              ...
              return (
                <button key={tab.id} type="button"
                  onClick={() => { setActiveTab(tab.id); setLabelData(null); if(tab.id==='customers') setSelectedCustomer(null); if(tab.id==='new_shipment') setReturnTab('new_shipment'); }}
                  ...
                  style={isActive ? {
                    ...
                    background: 'transparent', border: '1px solid transparent', color: 'rgba(196,181,253,0.6)'
                  }}>
                  <span className="text-base" style={isActive ? {color: tab.color === 'orange' ? '#fb923c' : tab.color === 'rose' ? '#fb7185' : '#a78bfa'} : {}}>{tab.icon}</span>
                  <span>{tab.label}</span>
                  {tab.badge > 0 && (
                    <span className="ml-auto text-white text-xs font-black px-2 py-0.5 rounded-full" style={{background: tab.color === 'orange' ? '#ea580c' : '#7c3aed'}}>{tab.badge}</span>
                  )}
                </button>
              );
            })}
        ...
        {activeTab === 'dashboard' && (
          <div className="max-w-7xl mx-auto">
            <div className="mb-8">
              ...
              <p className="text-sm" style={{color:'rgba(167,139,250,0.6)'}}>United Trade International Courier & Logistics — Real-time Overview</p>
            </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-purple-950/90 border border-purple-700/50 rounded-xl p-6 mb-6">
              ...
              <form onSubmit={handleSearchTracking} className="flex gap-3">
                <input className="flex-1 bg-purple-900/60 border border-purple-700/50 rounded-lg p-3 text-white text-sm focus:outline-none focus:border-purple-500" placeholder="Tracking daalo e.g. UT-123456789" value={trackingAwb} onChange={(e) => setTrackingAwb(e.target.value)} required />
            ...
            {selectedAwbShipment && (
              <>
                ...
                <div className="bg-purple-950/90 border border-purple-700/50 rounded-xl p-6 mb-6">
                  ...
                  <form onSubmit={handleAddTrackingUpdate} className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select className="w-full bg-purple-900/60 border border-purple-700/50 rounded-lg p-3 text-white text-sm focus:outline-none focus:border-purple-500" value={newUpdate.status} onChange={(e) => setNewUpdate({...newUpdate, status: e.target.value})}>
                    ...
                    <div>
                      ...
                      <input className="w-full bg-purple-900/60 border border-purple-700/50 rounded-lg p-3 text-white text-sm focus:outline-none focus:border-purple-500" placeholder="e.g. Paris, France" value={newUpdate.location} onChange={(e) => setNewUpdate({...newUpdate, location: e.target.value})} required />
                    </div>
                    <div>
                      <label className="text-xs text-purple-300 mb-1 block">Description / Note</label>
                      <input className="w-full bg-purple-900/60 border border-purple-700/50 rounded-lg p-3 text-white text-sm focus:outline-none focus:border-purple-500" placeholder="e.g. Package arrived at airport" value={newUpdate.description} onChange={(e) => setNewUpdate({...newUpdate, description: e.target.value})} />
                    </div>
                    <div>
                      <label className="text-xs text-purple-300 mb-1 block">Date & Time (optional)</label>
                      <input type="datetime-local" className="w-full bg-purple-900/60 border border-purple-700/50 rounded-lg p-3 text-white text-sm focus:outline-none focus:border-purple-500" value={newUpdate.custom_time} onChange={(e) => setNewUpdate({...newUpdate, custom_time: e.target.value})} />
                    </div>
                ...
                <div className="bg-purple-950/90 border border-purple-700/50 rounded-xl p-6">
                  <h3 className="font-bold text-purple-200 mb-5">📍 Live Tracking Timeline <span className="ml-2 text-xs bg-purple-900/50 text-purple-400 border border-purple-700 px-2 py-0.5 rounded-full font-normal">{trackingUpdates.length} updates</span></h3>
                  {trackingUpdates.length === 0 ? <p className="text-purple-300 text-sm text-center py-8">Abhi koi tracking update nahi hai</p> : (
                              ...
                              <div className={`flex-1 rounded-xl p-4 border ${isLatest ? 'bg-purple-900/60 border-slate-600' : 'bg-purple-900/20 border-purple-700/50'}`}>
                                <div className="flex items-start justify-between gap-2">
                                  <div className="flex-1">
                                    ...
                                    {update.description && <p className="text-purple-300 text-xs mt-1">{update.description}</p>}
                                    <p className="text-purple-400/70 text-xs mt-1.5">🕐 {new Date(update.created_at).toLocaleString('en-PK', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                                  </div>
        ...
        {activeTab === 'pending' && (
          <div className="max-w-6xl mx-auto">
            ...
            {pendingData.length === 0 ? (
              <div className="bg-purple-950/90 border border-purple-700/50 rounded-xl p-16 text-center"><p className="text-4xl mb-4">✅</p><p className="text-purple-300 font-medium">Koi pending request nahi hai</p></div>
            ) : (
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="bg-purple-950/90 border border-purple-700/50 rounded-xl p-6">
              <table className="w-full text-left border-collapse">
                <thead><tr className="text-purple-300 border-b border-purple-700/50 text-xs uppercase"><th className="pb-3 px-2">Name</th><th className="pb-3 px-2">Email</th><th className="pb-3 px-2">Phone</th><th className="pb-3 px-2">Joined</th><th className="pb-3 px-2 text-center">WhatsApp</th></tr></thead>
              ...
              <form onSubmit={handleSubmit} className="max-w-4xl mx-auto bg-purple-950/90 p-8 rounded-xl border border-purple-700/50 shadow-xl">
                ...
                <div className="mb-5 p-4 rounded-xl flex items-center gap-4" style={{background:'rgba(124,58,237,0.1)', border:'1px solid rgba(124,58,237,0.25)'}}>
                  <div className="flex-1">
                    ...
                    <input type="date" className="w-full bg-purple-900/60 border border-purple-500/30 rounded-lg p-2.5 text-white text-sm focus:outline-none focus:border-purple-500" value={formData.shipment_date} onChange={(e) => setFormData({...formData, shipment_date: e.target.value})} required />
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-purple-300 mb-1">Reset karo</p>
                    <button type="button" onClick={() => setFormData({...formData, shipment_date: todayStr()})} className="text-xs px-3 py-1.5 rounded-lg font-bold transition-all" style={{background:'rgba(124,58,237,0.2)', color:'#a78bfa', border:'1px solid rgba(124,58,237,0.3)'}}>🔄 Aaj ki Date</button>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div className="relative">
                    <input className="w-full bg-purple-900/60 p-2.5 rounded border border-purple-700/50 focus:outline-none focus:border-purple-500 text-sm text-white" placeholder="Sender Name" value={formData.sender_name}
                      onChange={(e) => {
                        const val = e.target.value;
                        setFormData({...formData, sender_name: val, sender_address: '', sender_phone: '', sender_email: ''});
                        if (val.length > 0) {
                          const unique = {};
                          ledgerData.forEach(item => { if (item.sender_name && item.sender_name.toLowerCase().startsWith(val.toLowerCase())) unique[item.sender_name] = item; });
                          setSenderSuggestions(Object.values(unique).slice(0, 5)); setShowSuggestions(true);
                        } else { setShowSuggestions(false); }
                      }}
                      onBlur={() => setTimeout(() => setShowSuggestions(false), 200)} required />
                    {showSuggestions && senderSuggestions.length > 0 && (
                      <div className="absolute z-50 w-full mt-1 rounded-xl overflow-hidden shadow-2xl" style={{background:'#1a0035', border:'1px solid rgba(139,92,246,0.4)'}}>
                        {senderSuggestions.map((s, i) => (
                          <button key={i} type="button" className="w-full text-left px-4 py-3 text-sm hover:bg-purple-700/40 transition-all border-b border-purple-800/30 last:border-0"
                            onClick={() => { setFormData({...formData, sender_name: s.sender_name, sender_address: s.sender_address || '', sender_phone: s.sender_phone || '', sender_email: s.sender_email || ''}); setShowSuggestions(false); }}>
        ...
        {activeTab === 'history' && (
          <div className="max-w-6xl mx-auto bg-purple-950/90 p-6 rounded-xl border border-purple-700/50 shadow-xl">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead><tr className="text-purple-300 border-b border-purple-700/50 text-sm"><th className="pb-3 px-2">AWB / Tracking</th><th className="pb-3 px-2">Sender</th><th className="pb-3 px-2">Receiver</th><th className="pb-3 px-2">Destination</th><th className="pb-3 px-2">Service</th><th className="pb-3 px-2 text-center">Action</th></tr></thead>
                <tbody className="text-sm">
                  {ledgerData.map((item) => (
          ...
          <div className="max-w-7xl mx-auto">
            {!selectedCustomer ? (
              <div className="bg-purple-950/90 p-6 rounded-xl border border-purple-700/50 shadow-xl">
                ...
                <table className="w-full text-left border-collapse">
                  <thead><tr className="text-purple-300 border-b border-purple-700/50 text-sm"><th className="pb-3 px-2">Customer Name</th><th className="pb-3 px-2">Phone</th><th className="pb-3 px-2">Email</th><th className="pb-3 px-2">Total Parcels</th><th className="pb-3 px-2 text-center">Action</th></tr></thead>
                  <tbody className="text-sm">
                ...
                <div className="bg-purple-950/90 p-6 rounded-xl border border-purple-700/50 shadow-xl">
                  <div className="flex justify-between items-center mb-6 border-b border-purple-800/40 pb-4">
                    ...
                    <div className="flex gap-2">
                      <button type="button" onClick={() => handleAddShipmentForCustomer(selectedCustomer)} className="text-white px-4 py-2 rounded-lg text-sm font-bold transition-all" style={{background:'linear-gradient(135deg, #7c3aed, #6d28d9)'}}>➕ Add Shipment</button>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="bg-purple-950/90 border border-purple-700/50 rounded-xl p-6 mb-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="bg-green-900/20 border border-green-500/30 rounded-xl p-4 text-center">
                    ...
                    <p className="text-2xl font-black text-green-400">Rs {vendors.reduce((s, v) => s + Number(v.total_paid || 0), 0).toLocaleString()}</p>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className={`border rounded-xl p-4 text-center ${baaki > 0 ? 'bg-rose-900/20 border-rose-500/30' : 'bg-green-900/20 border-green-500/30'}`}>
                        <p className={`text-xs font-bold mb-1 ${baaki > 0 ? 'text-rose-400' : 'text-green-400'}`}>{baaki > 0 ? '🔴 Baaki' : '✅ Clear'}</p>
                        <p className={`text-2xl font-black ${baaki > 0 ? 'text-rose-400' : 'text-green-400'}`}>Rs {Math.abs(baaki).toLocaleString()}</p>
                ...
                {totalDue > 0 && (() => {
                  ...
                  return (
                    <div className="mt-5">
                      <div className="flex justify-between text-xs text-purple-300 mb-1"><span>Payment Progress</span><span className="font-bold text-white">{pct}%</span></div>
                      <div className="w-full bg-slate-700 rounded-full h-3"><div className="bg-green-500 h-3 rounded-full transition-all" style={{ width: `${pct}%` }}></div></div>
                    </div>
                  );
                })()}
              </div>
              <div className="bg-purple-950/90 border border-purple-700/50 rounded-xl p-6 mb-4">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button type="button" onClick={() => setShowAddVendor(!showAddVendor)} className="bg-green-600 hover:bg-green-500 text-white font-bold px-4 py-2 rounded-lg transition-all text-sm">{showAddVendor ? '✕ Cancel' : '➕ Payment Add Karo'}</button>
                </div>
                {showAddVendor && (
                  <form onSubmit={handleAddVendor} className="bg-purple-900/50 rounded-xl p-4 mb-4 space-y-3">
                    <div><label className="text-xs text-purple-300 block mb-1">Payment Note *</label><input className="w-full bg-purple-900/60 border border-purple-700/50 rounded-lg p-3 text-white text-sm" placeholder="e.g. 1st installment..." value={vendorForm.name} onChange={(e) => setVendorForm({...vendorForm, name: e.target.value})} required /></div>
                    <div><label className="text-xs text-green-400 font-bold block mb-1">✅ Amount Paid (Rs)</label><input type="number" className="w-full bg-purple-900/60 border border-green-500/50 rounded-lg p-3 text-white text-sm" placeholder="e.g. 10000" value={vendorForm.total_paid} onChange={(e) => setVendorForm({...vendorForm, total_paid: e.target.value})} required /></div>
                    <div><label className="text-xs text-purple-400 font-bold block mb-1">📅 Payment Date</label><input type="date" className="w-full bg-purple-900/60 border border-purple-500/50 rounded-lg p-3 text-white text-sm" value={vendorForm.payment_date} onChange={(e) => setVendorForm({...vendorForm, payment_date: e.target.value})} required /></div>
                    <div><label className="text-xs text-purple-300 block mb-1">Notes (optional)</label><input className="w-full bg-purple-900/60 border border-purple-700/50 rounded-lg p-3 text-white text-sm" placeholder="Koi note..." value={vendorForm.notes} onChange={(e) => setVendorForm({...vendorForm, notes: e.target.value})} /></div>
                    {vendorMsg && <div className={`text-xs p-3 rounded-lg ${vendorMsg.startsWith('✅') ? 'bg-green-900/30 text-green-400 border border-green-700' : 'bg-red-900/30 text-red-400 border border-red-700'}`}>{vendorMsg}</div>}
                    <button type="submit" className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-lg transition-all">✅ Payment Save Karo</button>
                  </form>
                )}
                {vendors.length === 0 ? (
                  <div className="text-center py-8"><p className="text-3xl mb-2">💳</p><p className="text-purple-300 text-sm">Abhi koi payment entry nahi</p></div>
                ) : (
                          ...
                          {!isEditing ? (
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-xs text-purple-400/70 mt-0.5">{v.created_at ? new Date(v.created_at).toLocaleDateString('en-PK', { day: '2-digit', month: 'short', year: 'numeric' }) : ''}</p>
                              </div>
                              <div className="flex items-center gap-3">
                                <p className="text-xl font-black text-green-400">Rs {Number(v.total_paid || 0).toLocaleString()}</p>
                                <button type="button" onClick={() => handleEditVendor(v)} className="text-yellow-400 hover:text-yellow-300 text-xs font-bold px-2 py-1 rounded hover:bg-yellow-900/20 transition-all">✏️</button>
                                <button type="button" onClick={() => handleDeleteVendor(v.id)} className="text-red-400 hover:text-red-300 text-xs font-bold px-2 py-1 rounded hover:bg-red-900/20 transition-all">🗑️</button>
                              </div>
                            </div>
                          ) : (
                            <form onSubmit={handleUpdateVendor} className="space-y-3">
                              <div><label className="text-xs text-purple-300 block mb-1">Payment Label *</label><input className="w-full bg-purple-900/60 border border-purple-700/50 rounded-lg p-2.5 text-white text-sm" value={editVendorForm.name} onChange={(e) => setEditVendorForm({...editVendorForm, name: e.target.value})} required /></div>
                              <div><label className="text-xs text-green-400 font-bold block mb-1">✅ Amount Paid</label><input type="number" className="w-full bg-purple-900/60 border border-green-500/50 rounded-lg p-2.5 text-white text-sm" value={editVendorForm.total_paid} onChange={(e) => setEditVendorForm({...editVendorForm, total_paid: e.target.value})} /></div>
                              <div><label className="text-xs text-purple-300 block mb-1">Notes</label><input className="w-full bg-purple-900/60 border border-purple-700/50 rounded-lg p-2.5 text-white text-sm" value={editVendorForm.notes} onChange={(e) => setEditVendorForm({...editVendorForm, notes: e.target.value})} /></div>
                              <div className="flex gap-2">
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-purple-950/90 p-6 rounded-xl border border-purple-700/50 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleUpdate} className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><label className="text-xs text-purple-300">Sender Address</label><input className="w-full bg-purple-900/60 p-2 mt-1 rounded border border-purple-700/50 text-white" value={editFormData.sender_address} onChange={(e) => setEditFormData({...editFormData, sender_address: e.target.value})} /></div>
              <div><label className="text-xs text-purple-300">Receiver Address</label><input className="w-full bg-purple-900/60 p-2 mt-1 rounded border border-purple-700/50 text-white" value={editFormData.receiver_address} onChange={(e) => setEditFormData({...editFormData, receiver_address: e.target.value})} /></div>
              <div><label className="text-xs text-purple-300">Sender Phone</label><input className="w-full bg-purple-900/60 p-2 mt-1 rounded border border-purple-700/50 text-white" value={editFormData.sender_phone} onChange={(e) => setEditFormData({...editFormData, sender_phone: e.target.value})} /></div>
              <div><label className="text-xs text-purple-300">Receiver Phone</label><input className="w-full bg-purple-900/60 p-2 mt-1 rounded border border-purple-700/50 text-white" value={editFormData.receiver_phone} onChange={(e) => setEditFormData({...editFormData, receiver_phone: e.target.value})} /></div>
              <div><label className="text-xs text-purple-300">Sender Email</label><input className="w-full bg-purple-900/60 p-2 mt-1 rounded border border-purple-700/50 text-white" value={editFormData.sender_email} onChange={(e) => setEditFormData({...editFormData, sender_email: e.target.value})} /></div>
              <div><label className="text-xs text-purple-300">Receiver Email</label><input className="w-full bg-purple-900/60 p-2 mt-1 rounded border border-purple-700/50 text-white" value={editFormData.receiver_email} onChange={(e) => setEditFormData({...editFormData, receiver_email: e.target.value})} /></div>
